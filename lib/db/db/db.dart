...
//import 'dart:nativewrappers/_internal/vm/lib/object_patch.dart';

import 'package:kelimekesfi1/db/models/lists.dart';
import 'package:kelimekesfi1/db/models/words.dart';
...
class DB {
  ...
  Future<Database> get database async {
    ...
    _database = await _initDB('kelimekesfi.db');
    return _database!;
  }
  ...
  Future<Lists> insertList(Lists list) async {
    final db = await instance.database;
    final id = await db.insert(tableNameLists, list.toJson());
  ...
  Future<List<Map<String, Object?>>> readListsAll() async {
    ...
    final lists = await db.rawQuery('SELECT id, name FROM $tableNameLists');

    for (var element in lists) {
      element =element as Map<String, Object?>;
      var wordInfoByList = await db.rawQuery('''
      SELECT 
        (SELECT COUNT(*) FROM $tableNameWords WHERE list_id = ${element['id']}) as sum_word,
        (SELECT COUNT(*) FROM $tableNameWords WHERE status = 0 AND list_id = ${element['id']}) as sum_unlearned
      ''');

      var temp = Map.of(wordInfoByList[0]);
